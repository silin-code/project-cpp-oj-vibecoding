...
- [ ] 1.4 创建数据库 `oj_system` + 执行 `database/init.sql` 建表
- [ ] 1.5 插入 root 管理员账号（密码 bcrypt 哈希）

## 阶段二：项目骨架

- [ ] 2.1 根目录 `CMakeLists.txt`（C++17, 链接 mysqlclient/seccomp/ssl, 含 tests 子目录）
- [ ] 2.2 `config/config.json`（端口 8080, MySQL 连接信息, Session 路径, 判题参数）
- [ ] 2.3 `.gitignore`（build/, *.o, .vscode/ 等）
- [x] 2.4 `third_party/` 下载 cpp-httplib + nlohmann-json（已完成）

## 阶段三：基础设施层

- [ ] 3.1 `src/utils/logger.h` + `logger.cc`（日志级别 DEBUG/INFO/WARN/ERROR, 文件输出）
- [ ] 3.2 `src/utils/config.h` + `config.cc`（读取 config.json, 提供 getter）
- [ ] 3.3 `src/db/connection_pool.h` + `connection_pool.cc`（MySQL 连接池, 最大连接数可配）
- [ ] 3.4 `src/model/problem.h`（Problem 结构体）
- [ ] 3.5 `src/model/test_case.h`（TestCase 结构体）
- [ ] 3.6 `src/model/user.h`（User 结构体）

## 阶段四：后端 Service 层

- [ ] 4.1 `src/service/auth_service.h` + `auth_service.cc`
  - 注册: 用户名唯一性检查, bcrypt 哈希存密码
  - 登录: 验证密码, 生成 Session ID, 写 Session 文件到 `/var/oj/sessions/`
  - 鉴权: 根据 Cookie Session ID 读取 Session 文件, 返回 user_id/role
  - 限流: 内存中记录每用户最后提交时间, 10 秒窗口
  - Session 清理: 启动 + 每 30 分钟扫描过期文件
- [ ] 4.2 `src/service/problem_service.h` + `problem_service.cc`
  - CRUD 题目（增删改查 + 列表含通过率统计）
  - 查测试用例（按 problem_id + 区分 sample/hidden）
- [ ] 4.3 `src/service/executor_service.h` + `executor_service.cc`
  - 任务队列 + Worker 线程
...
- [ ] 7.1 `tests/unit/problem_test.cc`（题目服务单元测试, Google Test）
- [ ] 7.2 `tests/unit/executor_test.cc`（沙箱判题单元测试）
- [ ] 7.3 `tests/integration/api_test.cc`（端到端 API 测试, 注册→登录→建题→提交→查结果）
- [ ] 7.4 `scripts/start.sh`（编译 + 运行后端）
- [ ] 7.5 systemd service 文件（开机自启, 日志收集）
