...
g++ $CXXFLAGS -c "executor_test.cc"                      -o "$BUILD_DIR/executor_test.o"

echo "==> Linking..."
g++ "$BUILD_DIR"/{logger,config,connection_pool,problem_service,executor_service,executor_test}.o $LDFLAGS -o "$BUILD_DIR/executor_test"

echo "==> Running (working dir: $PROJ)..."
cd "$PROJ"
"$BUILD_DIR/executor_test" "$@"
