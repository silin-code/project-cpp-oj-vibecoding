...
echo "==> Compiling problem_api_test..."
...
g++ $CXXFLAGS -c "$PROJ/src/handler/problem_handler.cc"  -o "$BUILD_DIR/problem_handler.o"
g++ $CXXFLAGS -c "problem_api_test.cc"                   -o "$BUILD_DIR/problem_api_test.o"

echo "==> Linking..."
g++ "$BUILD_DIR"/{logger,config,connection_pool,auth_service,auth_handler,problem_service,problem_handler,problem_api_test}.o $LDFLAGS -o "$BUILD_DIR/problem_api_test"

echo "==> Running (working dir: $PROJ)..."
cd "$PROJ"
"$BUILD_DIR/problem_api_test" "$@"
