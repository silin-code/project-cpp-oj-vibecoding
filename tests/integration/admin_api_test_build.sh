...
g++ $CXXFLAGS -c "$PROJ/src/handler/admin_handler.cc"    -o "$BUILD_DIR/admin_handler.o"
g++ $CXXFLAGS -c "admin_api_test.cc"                     -o "$BUILD_DIR/admin_api_test.o"

echo "==> Linking..."
g++ "$BUILD_DIR"/{logger,config,connection_pool,auth_service,auth_handler,problem_service,admin_handler,admin_api_test}.o $LDFLAGS -o "$BUILD_DIR/admin_api_test"

echo "==> Running (working dir: $PROJ)..."
cd "$PROJ"
"$BUILD_DIR/admin_api_test" "$@"
