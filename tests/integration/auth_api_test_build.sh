...
g++ $CXXFLAGS -c "auth_api_test.cc"                   -o "$BUILD_DIR/auth_api_test.o"

echo "==> Linking..."
g++ "$BUILD_DIR"/{logger,config,connection_pool,auth_service,auth_handler,auth_api_test}.o $LDFLAGS -o "$BUILD_DIR/auth_api_test"

echo "==> Running (working dir: $PROJ)..."
cd "$PROJ"
"$BUILD_DIR/auth_api_test" "$@"
